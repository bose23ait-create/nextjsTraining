...
type Order = { _id: string; createdAt: string; total: number; status: OrderStatus; paymentStatus?: 'pending' | 'paid' | 'failed'; items: Array<{ productId: string; name: string; price: number; quantity: number }>; customerDetails?: { name: string; email: string; phone: string; address: string; city: string; state: string; postalCode: string } };
...
export default function AdminOrdersPage() {
  ...
  return <Box sx={{ minHeight: '100vh', bgcolor: '#f4f6f9', py: { xs: 2, md: 4 } }}><Container maxWidth="lg"><Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}><Typography variant="h4" sx={{ fontWeight: 800 }}>Manage orders</Typography><Button onClick={() => router.push('/admin')}>Products</Button></Box>{error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}{loading && <Box><Skeleton variant="rounded" height={180} /><Skeleton variant="rounded" height={180} sx={{ mt: 2 }} /></Box>}{!loading && !error && orders.length === 0 && <Box sx={{ bgcolor: '#fff', p: 5, textAlign: 'center', border: '1px solid #e0e5eb' }}><Typography>No orders found</Typography></Box>}{!loading && orders.map((order) => <Box key={order._id} sx={{ bgcolor: '#fff', p: { xs: 2, md: 3 }, mb: 2, border: '1px solid #e0e5eb' }}><Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 2, flexWrap: 'wrap', mb: 1 }}><Typography sx={{ fontWeight: 800 }}>Order #{order._id.slice(-8)}</Typography><Typography color="text.secondary">{new Date(order.createdAt).toLocaleString()}</Typography></Box>{order.customerDetails && <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>{order.customerDetails.name} | {order.customerDetails.email} | {order.customerDetails.phone}<br />{order.customerDetails.address}, {order.customerDetails.city}, {order.customerDetails.state} {order.customerDetails.postalCode}</Typography>}{order.items.map((item) => <Box key={`${order._id}-${item.productId}`} sx={{ display: 'flex', justifyContent: 'space-between', py: 0.75 }}><Typography>{item.name} x {item.quantity}</Typography><Typography>₹{(item.price * item.quantity).toFixed(2)}</Typography></Box>)}<Divider sx={{ my: 1 }} /><Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 2 }}><Box sx={{ display: 'flex', gap: 1 }}><Chip label={order.status} color={order.status === 'completed' ? 'success' : 'default'} size="small" />{order.paymentStatus && <Chip label={order.paymentStatus === 'paid' ? 'Payment Completed' : 'Payment Pending'} color={order.paymentStatus === 'paid' ? 'success' : 'warning'} size="small" />}</Box><Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><Typography sx={{ fontWeight: 800 }}>₹{order.total.toFixed(2)}</Typography><select value={order.status} disabled={savingId === order._id} onChange={(event) => void updateStatus(order._id, event.target.value as OrderStatus)} aria-label={`Update status for order ${order._id}`} style={{ height: 36, padding: '0 8px' }}>{statuses.map((status) => <option key={status} value={status}>{status}</option>)}</select></Box></Box></Box>)}</Container></Box>;
}
